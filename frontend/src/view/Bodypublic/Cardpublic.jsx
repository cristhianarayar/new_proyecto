...
const Cardpublic = (produc) => {
  
  const [show, setShow] = useState(false);
  const {getDetPub} = useContext(MicarContext)

  const handleShow = async (id) => {
    setShow(true)
    const data = await getDetPub(id)
    ...
    <>
        <Card style={{ width: "18rem" }}>
          ...
          <Card.Body>
            ...
            <Button variant="dark" onClick={() => handleShow(produc.prod.id_prod)} className="ver" >Ver Detalle</Button>
          </Card.Body>
        </Card>
